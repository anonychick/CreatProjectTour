(function ($) {
    ...
    var GmzSearchHotel = {
        ...
        init: function (el) {
            ...
            this.renderEl = $('.list-hotel__content', this.container);
        ...
        _initEventSearch: function(){
            ...
            $('.star-item input', base.container).on('change', function () {
                var t = $(this);
                var star = [];
                $('.star-item input', base.container).each(function(){
                    var me = $(this);
                    if(me.is(':checked')){
                        star.push(me.val());
                    }
                ...
                if(star.length){
                    base.dataFilter['hotel_star'] = star.toString();
                }else{
                    base.dataFilter['hotel_star'] = '';
                }

                base._resetPage();
                base._initSearch();
                base._pushStateToFilter('hotel_star', star.toString());
        ...
        _initSearch: function(){
            ...
            base.xhrRequest = $.post(base.container.data('action'), base.dataFilter, function (respon) {
                ...
                if($('.list-half-map__left .list-hotel__content').length) {
                    $('.list-hotel__content', parentImage).find('img').one("load", function () {
                        $(this).addClass('loaded');
                        if ($('.list-hotel__content', parentImage).find('img.loaded').length === $('.list-hotel__content', parentImage).find('img').length) {
                            setTimeout(function () {
    ...
    $(document).ready(function() {
        GmzSearchHotel.init('.gmz-search-result');
    });
})(jQuery);