...
@section('title', __(sprintf('New %s', $taxonomy->taxonomy_title)))
    ...
    <div class="layout-top-spacing">
        <div class="statbox widget box box-shadow">
                ...
                <div class="row">
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <div class="d-flex align-items-center justify-content-between">
                            <h4>{{__(sprintf('New %s', $taxonomy->taxonomy_title))}}</h4>
                            <a href="{{dashboard_url('term/' . $taxonomy->taxonomy_name)}}" class="btn btn-dark">{{__('Back to all')}}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <form class="gmz-form-action form-translation" action="{{$data['action']}}" method="POST" data-loader="body" enctype="multipart/form-data">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    <a href="{{dashboard_url('term/' . $data['tax_name'])}}" class="btn btn-dark">{{__('Discard')}}</a>
