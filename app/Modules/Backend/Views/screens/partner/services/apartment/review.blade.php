...
@section('title', __('Apartment Reviews'))
    ...
    <div class="layout-top-spacing">
        <div class="statbox widget box box-shadow">
            ...
            <div class="filter-action pl-2 mt-3">
                ...
                @endphp
                <a href="{{dashboard_url('apartment-review')}}" class="@if($status == '') text-primary font-weight-bold @endif mr-2">{{__('All')}}</a>
                <a href="{{dashboard_url('apartment-review?status=publish')}}" class="@if($status == 'publish') text-primary font-weight-bold @endif mr-2">{{__('Publish')}}</a>
                <a href="{{dashboard_url('apartment-review?status=pending')}}" class="@if($status == 'pending') text-primary font-weight-bold @endif">{{__('Pending')}}</a>
            </div>
                        ...
                        @foreach($allPosts->items() as $key => $item)
                            ...
                            <tr>
                                <td style="max-width: 450px">
                                    ...
                                    <p>{{__('In:')}} &nbsp;<b><a href="{{get_apartment_permalink($post['post_slug'])}}" target="_blank">{{get_translate($post['post_title'])}}</a></b></p>
                                    <p>
