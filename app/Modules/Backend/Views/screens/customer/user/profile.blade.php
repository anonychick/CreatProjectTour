    ...
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <h4>
                {{__('Your profile')}}
                @if($data['request'] != 1)
                    <a href="{{dashboard_url('profile?action=become-partner')}}" class="btn btn-outline-primary btn-sm ml-2">{{__('Become to Partner')}}</a>
                @endif
            </h4>
            @if($data['request'] == 1)
                <div class="alert alert-warning d-flex align-items-center mt-4">
                    {{__('Your request to become a partner is waiting for administrator approval.')}}
                    <a href="{{dashboard_url('profile?action=cancel-become-partner')}}" class="btn btn-outline-danger btn-sm ml-2">{{__('Cancel')}}</a>
                </div>
            @endif

            @if(session()->has('profile_message'))
                <div class="alert alert-success @if($data['request'] == 1) mt-0 @else mt-4 @endif">
                {{session('profile_message')}}
                </div>
            @endif

            <form class="gmz-form-action @if($data['request'] == 1) mt-4 @else mt-5 @endif" action="{{dashboard_url('update-profile')}}" method="POST" autocomplete="off">
