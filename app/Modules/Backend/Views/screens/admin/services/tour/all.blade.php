                        ...
                        <td class="d-flex align-items-center">
                            ...
                            <div>
                                ...
                                <div class="quick-action">
                                    ...
                                    @endphp
                                    @if($status == 'trash')
                                        <a class="gmz-link-action" href="javascript:void(0);" data-action="{{dashboard_url('restore-tour')}}" data-params="{{base64_encode(json_encode($params))}}" data-remove-el="tr">{{__('Restore')}}</a>
                                        <a class="text-danger gmz-link-action" href="javascript:void(0);" data-confirm="true" data-action="{{dashboard_url('hard-delete-tour')}}" data-params="{{base64_encode(json_encode($params))}}" data-remove-el="tr">{{__('Delete Permanently')}}</a>
                                    @else
                        ...
                        <td class="text-center">
                            <div class="dropdown custom-dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink1">
                                    @if($status == 'trash')
                                        <a class="dropdown-item gmz-link-action text-info" href="javascript:void(0);" data-action="{{dashboard_url('restore-tour')}}" data-params="{{base64_encode(json_encode($params))}}" data-remove-el="tr">{{__('Restore')}}</a>
                                        <a class="dropdown-item text-danger gmz-link-action" href="javascript:void(0);" data-confirm="true" data-action="{{dashboard_url('hard-delete-tour')}}" data-params="{{base64_encode(json_encode($params))}}" data-remove-el="tr">{{__('Delete Permanently')}}</a>
                                    @else
                                        ...
                                        @foreach(list_service_status($item->status) as $status => $title)
                                            ...
                                            @endphp
                                            <a class="dropdown-item gmz-link-action text-warning" href="javascript:void(0);" data-action="{{dashboard_url('change-tour-status')}}" data-params="{{base64_encode(json_encode($paramsTemp))}}">{{$title}}</a>
                                        @endforeach
