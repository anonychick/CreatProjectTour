...
@if(is_enable_service('apartment'))
    <div class="gmz-add-menu-box overflow-hidden">
        <h5 class="title d-flex align-items-center justify-content-between">{{__('Apartment')}} {!! get_icon('icon_system_arrow_down') !!}</h5>
            ...
            <div class="content">
                ...
                <div class="checkbox  checkbox-success mb-2">
                    <input type="checkbox"
                           ...
                           value="{{ $item->id }}"
                           data-id="{{ $item->id  }}"
                           data-url="{{ get_apartment_permalink($item->post_slug)  }}"
                           data-type="apartment"
                           data-name="{{ $post_title }}"
                           id="menu_item_apartment_{{ $item->id  }}"/>
                    <label for="menu_item_apartment_{{ $item->id  }}">{{ $post_title }}</label>
                </div>
				<?php
				}
				}else {
