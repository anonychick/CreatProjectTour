...
@if(is_enable_service('car'))
    <div class="gmz-add-menu-box overflow-hidden">
        <h5 class="title d-flex align-items-center justify-content-between">{{__('Car')}} {!! get_icon('icon_system_arrow_down') !!}</h5>
        <div class="menu-content-wrapper">
            <div class="content">
				<?php
				$posts = get_posts(['post_type' => 'car']);
				if(!$posts->isEmpty()){
                ...
                <div class="checkbox  checkbox-success mb-2">
                    <input type="checkbox"
                           ...
                           value="{{ $item->id }}"
                           data-id="{{ $item->id  }}"
                           data-url="{{ get_car_permalink($item->post_slug)  }}"
                           data-type="car"
                           data-name="{{ $post_title }}"
                           id="menu_item_car_{{ $item->id  }}"/>
                    <label for="menu_item_car_{{ $item->id  }}">{{ $post_title }}</label>
                </div>
				<?php
				}
				}else {
