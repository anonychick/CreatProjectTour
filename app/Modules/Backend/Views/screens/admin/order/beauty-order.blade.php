...
@section('title', __('Beauty Orders'))
                    ...
                    <table id="gmz_table" class="multi-table table table-striped table-bordered table-hover non-hover w-100" data-plugin="footable">
                        ...
                        @foreach($allPosts->items() as $key => $item)
                            ...
                            <tr>
                                ...
                                <td>
                                    <span>{{date(get_date_format(), $data['cart_data']['check_in'])}}</span>
                                    <br />
                                    <span>{{date(get_time_format(), $data['cart_data']['check_in'])}}</span>
                                </td>
                                {{-- Total --}}
                                <td>
                                ...
                                <td>
                                    ...
                                    {{--Action button--}}
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-warning gmz-open-modal"
                                        ...
                                        <div class="dropdown-menu" style="will-change: transform;">
                                            @foreach(list_order_status($item['status']) as $status => $title)
                                                <a class="dropdown-item btnChangeStatus" href="javascript:void(0);"
                                                   data-action="{{$action_change_status}}"
                                                   data-params="{{$data_params}}"
                                                   data-change-status="{{$status}}">
                                                    {{$title}}
                                                </a>
                                            @endforeach
                                                @action('gmz_order_group_actions', $item)
                                        </div>
                                    </div>
