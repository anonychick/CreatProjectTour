    ...
    <form id="form_filter" action="{{current_url()}}" method="get">
        ...
        <div class="form-row">
            ...
            @if(!empty($get_data['search']) || !empty($get_data['filter_status']))
                <a class="btn btn-sm btn-danger bs-tooltip" style="height: 30px" title="{{__("Clear Filter")}}" href="{{dashboard_url("order/".$post_type['post_type']."/")}}"><i class="fas fa-times mr-1"></i>{{__("Clear")}}</a>
            @endif
        </div>
    </form>
</div>
