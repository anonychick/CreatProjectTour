...
<div class="gmz-email-wrapper">
    ...
    <div class="email-content">
        @php
            $tour_name = get_translate($post['post_title']);
        @endphp
        <p>{{__('Hello')}} <strong>{{get_user_name($user_id)}}</strong>,</p>
        <p><b>{{sprintf(__('Have new booking request for Tour "%s"'), $tour_name)}}</b></p>
        <p><b>{{__('Booking detail')}}</b></p>
        <div class="order-detail">
            <div class="item">
                <span class="title">{{__('Tour ID:')}}</span>
                <span class="info">{{ $post['id'] }}</span>
            </div>
            <div class="item">
                <span class="title">{{__('Tour Name:')}}</span>
                <span class="info"><a href="{{get_tour_permalink($post['post_slug'])}}">{{$tour_name}}</a></span>
            </div>
            <div class="item">
