...
<div class="gmz-email-wrapper">
    ...
    <div class="email-content">
        @php
            $apartment_name = get_translate($post['post_title']);
        @endphp
        <p>{{__('Hello')}} <strong>{{get_user_name($user_id)}}</strong>,</p>
        <p><b>{{sprintf(__('Have new booking request for Apartment "%s"'), $apartment_name)}}</b></p>
        <p><b>{{__('Booking detail')}}</b></p>
        <div class="order-detail">
            <div class="item">
                <span class="title">{{__('Apartment ID:')}}</span>
                <span class="info">{{ $post['id'] }}</span>
            </div>
            <div class="item">
                <span class="title">{{__('Apartment Name:')}}</span>
                <span class="info"><a href="{{get_apartment_permalink($post['post_slug'])}}">{{$apartment_name}}</a></span>
            </div>
            <div class="item">
