@php
    ...
    $user_name = get_user_name($post_data['user_id']);
    $css = '
           .text-center{
               text-align: center;
           }
...
<div class="gmz-email-wrapper">
    ...
    <div class="email-content">
        <p>{{__('Hello')}} <strong>{{$user_name}}</strong>,</p>
        <p><b>{{__('Your account have been created on site. You can login to use the website now. Wish you have a pleasant experience.')}}</b></p>
        <div class="email-action">
            <a class="btn-primary" target="_blank" href="{{url('login')}}">{{__('Login Now')}}</a>
        </div>
    </div>
