...
<div class="gmz-email-wrapper">
    ...
    <div class="email-content">
        ...
        <p><b>{{__('A new order has been created on the site')}}</b></p>
        <div class="order-detail" style="margin-top: 20px;">
            ...
            <div class="item">
                ...
                <span class="info"><a href="{{get_hotel_permalink($post['post_slug'])}}">{{get_translate($post['post_title'])}}</a></span>
            </div>
            <div class="item">
            ...
            <div class="item">
                <span class="title">{{__('Check In')}}</span>
                <span class="info">{{date(get_date_format(), $order['start_date'])}}</span>
            </div>
            <div class="item">
                <span class="title">{{__('Check Out')}}</span>
                <span class="info">{{date(get_date_format(), $order['end_date'])}}</span>
            </div>
            <div class="item">
                <span class="title">{{__('Number Of Days')}}</span>
                <span class="info">{{$checkout_data['cart_data']['number_day']}}</span>
            </div>
            <div class="item">
                <span class="title">{{__('Total Rooms')}}</span>
                <span class="info">{{$checkout_data['cart_data']['number']}}</span>
            </div>
            @if(!empty($checkout_data['cart_data']['adult']))
                <div class="item">
                    <span class="title">{{__('Number of Adult')}}</span>
                    <span class="info">{{$checkout_data['cart_data']['adult']}}</span>
                </div>
            @endif
            @if(!empty($checkout_data['cart_data']['children']))
                <div class="item">
                    <span class="title">{{__('Number of Children')}}</span>
                    <span class="info">{{$checkout_data['cart_data']['children']}}</span>
                </div>
            @endif
            @if(!empty($checkout_data['cart_data']['rooms']))
                <div class="item">
                    <span class="title">{{__('Room Details')}}</span>
                    <div class="info">
                ...
                <div class="item">
                    ...
                    <div class="info">
                        @foreach($extras as $item)
                            <div>{{get_translate($item['title'])}}: {{convert_price($item['price'])}}</div>
                        @endforeach
                    </div>
                </div>
            @endif
        ...
        <div class="email-action">
            <a class="btn-primary" target="_blank" href="{{dashboard_url('order/hotel')}}">{{__('Manage Order')}}</a>
        </div>
    </div>
