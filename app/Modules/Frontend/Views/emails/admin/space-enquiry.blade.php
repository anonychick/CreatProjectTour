...
<div class="gmz-email-wrapper">
    ...
    <div class="email-content">
        @php
            $space_name = get_translate($post['post_title']);
        @endphp
        <p>{{__('Hello')}} <strong>{{get_user_name($user_id)}}</strong>,</p>
        <p><b>{{sprintf(__('Have new booking request for Space "%s"'), $space_name)}}</b></p>
        <p><b>{{__('Booking detail')}}</b></p>
        <div class="order-detail">
            <div class="item">
                <span class="title">{{__('Space ID:')}}</span>
                <span class="info">{{ $post['id'] }}</span>
            </div>
            <div class="item">
                <span class="title">{{__('Space Name:')}}</span>
                <span class="info"><a href="{{get_space_permalink($post['post_slug'])}}">{{$space_name}}</a></span>
            </div>
            <div class="item">
