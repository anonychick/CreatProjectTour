    ...
    <div class="booking-form__content">
        ...
        @if($booking_form == 'both' || $booking_form == 'instant')
           <div class="tab-pane fade show active" id="instant" role="tabpanel" aria-labelledby="instant-tab">
               ...
               @else
                    <form class="gmz-form-action booking-form-single" action="{{ url('apartment-add-cart') }}" method="POST" data-price-url="{{url('apartment-get-real-price')}}">
            <input type="hidden" name="post_type" value="{{GMZ_SERVICE_APARTMENT}}"/>
            <input type="hidden" name="post_id" value="{{$post['id']}}"/>
            <input type="hidden" name="post_hashing" value="{{gmz_hashing($post['id'])}}"/>
            @include('Frontend::components.loader')
            <div class="booking-date @if($booking_type == 'per_day') range @else single has-time @endif" data-date-format="{{get_date_format_moment()}}" data-action="{{ url('apartment-fetch-calendar-availability') }}" data-id="{{$post['id']}}" data-hashing="{{gmz_hashing($post['id'])}}" data-post_type="{{GMZ_SERVICE_APARTMENT}}" data-booking_type="{{$post['booking_type']}}" data-action-time="{{url('apartment-fetch-time')}}">
                <label>{{__('Date')}}</label>
                @if($booking_type == 'per_day')
                <input type="text" class="input-hidden date-input" name="check_in_out" value="" data-same-date="false"/>
        ...
        @if($booking_form == 'both' || $booking_form == 'enquiry')
           <div class="tab-pane fade show" id="enquiry" role="tabpanel" aria-labelledby="enquiry-tab">
               <form class="gmz-form-action enquiry-form-single" action="{{ url('apartment-send-enquiry') }}" method="POST">
                   <input type="hidden" name="post_id" value="{{$post['id']}}"/>
                   <input type="hidden" name="post_hashing" value="{{gmz_hashing($post['id'])}}"/>
                @include('Frontend::components.loader')
