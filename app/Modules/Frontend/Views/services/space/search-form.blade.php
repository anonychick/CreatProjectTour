@php
   ...
   if($has_advanced){
       $price_range = get_price_range('space');
       $space_types = get_terms('name','space-type');
       $space_amenity = get_terms('name','space-amenity');
                    ...
                    @if(!empty($space_types))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            ...
                            @foreach($space_types as $key => $type)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item" name="space_types[]" value="{{$key}}"><span>{{get_translate($type)}}</span></label>
                            @endforeach
                            <input type="hidden" name="space_type" value=""/>
                        </div>
                    @endif

                    @if(!empty($space_amenity))
                        <div class="col-md-6 gmz-checkbox-wrapper">
                            <div class="search-form__label">{{__('Amenities')}}</div>
                            @foreach($space_amenity as $key => $value)
                                <label class="checkbox-inline"><input type="checkbox" class="gmz-checkbox-item" name="space_amenities[]" value="{{$key}}"><span>{{get_translate($value)}}</span></label>
                            @endforeach
