@php
    ...
    $link = get_tour_permalink($post_object['post_slug']);
...
<div class="cart-info">
    ...
    <ul class="cart-info__meta">
        <li>
            ...
            @if($booking_type == 'package')
                <span class="value">{{date(get_date_format(), $cart['cart_data']['check_in'])}} - {{date(get_date_format(), $cart['cart_data']['check_out'])}}</span>
            @else
        ...
        @endif
        @action('gmz_tour_cart_item_meta', $cart)
    </ul>
    @php
    ...
    <div class="cart-info__price">
        <ul>
            ...
            @action('gmz_tour_cart_item_subtotal_after', $cart)
