...
@endphp
<div class="car-item car-item--list" data-id="{{$item->id}}" data-lat="{{$item->location_lat}}" data-lng="{{$item->location_lng}}">
    <div class="row">
        <div class="col-4">
            <div class="car-item__thumbnail">
                ...
                @if($item->is_featured == 'on')
                    <p class="car-item__label">{{__('Featured')}}</p>
                @endif
                @if(!empty($type))
                    @php
        ...
        <div class="col-8">
            <div class="car-item__details">
                ...
                @if(!empty($location))
                    <p class="car-item__location"><i class="fas fa-map-marker-alt mr-2"></i>{{$location}}</p>
                @endif
                ...
                <div class="d-flex justify-content-between align-items-center mt-auto">
                    ...
                    <a class="btn btn-primary" href="{{get_car_permalink($item->post_slug)}}">{{__('View Detail ')}}</a>
                </div>
            </div>
        </div>
    </div>
