...
class ApartmentService extends AbstractService
{
    ...
    {
        $this->repository = ApartmentRepository::inst();
    }
    ...
    public function fetchTime(Request $request)
    {
        ...
        if ($start_date && gmz_compare_hashing($postID, $postHashing)) {
            ...
            $orderData = $orderRepo->getOrderItems($postID, $start_date, $start_date, GMZ_SERVICE_APARTMENT);
    ...
    public function sendEnquiry(Request $request)
    {
        ...
        $post_object = get_post($postID, GMZ_SERVICE_APARTMENT);
    ...
    public function getRealPrice($post_id, $check_in, $check_out, $extras)
    {
        ...
        if ($booking_type == 'per_day') {
            ...
            $avails = $apartmentAvaiRepo->getDataAvailability($post_id, $check_in, $check_out);
            ...
            $orderRepo = OrderRepository::inst();
            $orders = $orderRepo->getOrderItemsWithTime($post_id, $check_in, $check_out, $startTime, $endTime, 'apartment', $booking_type);
    ...
    public function fetchApartmentAvailability(Request $request)
    {
        ...
        if ($start_date && $end_date && gmz_compare_hashing($postID, $postHashing)) {
            $apartmentAvaiRepo = ApartmentAvailabilityRepository::inst();
            $orderRepo = OrderRepository::inst();
            $avails = $apartmentAvaiRepo->getDataAvailability($postID, $start_date, $end_date);
            $apartment = $this->repository->find($postID);
            $price = (float)$apartment->base_price;
            if ($bookingType == 'per_day') {
                $orders = $orderRepo->getOrderItems($postID, $start_date, $end_date, GMZ_SERVICE_APARTMENT);
            } else {
                $orders = $orderRepo->getOrderItemsGroupDate($postID, $start_date, $end_date, GMZ_SERVICE_APARTMENT);
            }
    ...
    public function addToCart(Request $request)
    {
        ...
        if (gmz_compare_hashing($post_id, $post_hashing) && $check_in && $check_out) {
            $apartment_object = $this->repository->find($post_id);
            $booking_type = 'per_day';
            ...
            if (intval($adult) + intval($children) > $apartment_object['number_of_guest']) {
                return [
                    'status' => false,
                    'message' => sprintf(__('The maximum number of guests is %s'), $apartment_object['number_of_guest'])
                ];
            }
            ...
            $extra_data = $this->getExtraPrice($apartment_object['extra_services'], $extra);
            ...
            if ($price_data['status']) {
                ...
                $cart_data = [
                    'post_id' => $post_id,
                    'post_object' => serialize($apartment_object),
                    'post_type' => 'apartment',
                    'base_price' => $base_price,
    ...
    public function getSearchResult(Request $request)
    {
        ...
        $default = [
            ...
            'infant' => '',
            'bookingType' => '',
            'number' => intval(get_option('apartment_search_number', 6)),
            'unavailable_id' => '',
            'layout' => 'list',
            'sort' => 'new'
        ];
        ...
        if (!empty($params['checkIn']) && !empty($params['checkOut'])) {
            $unavail = $apartmentAvaiRepo->getListUnavailable($params);
        ...
        $search_str = get_search_string('apartment', $total_result, $params);
        $location = [];
        ...
        if ($total_result > 0) {
            foreach ($data->items() as $key => $val) {
                ...
                $item = [
                    'id' => $val['id'],
                    'url' => get_apartment_permalink($val['post_slug']),
                ...
                if ($params['layout'] == 'list') {
                    $html .= view('Frontend::services.apartment.items.list-item', ['item' => $val])->render();
                } else {
                    $html .= '<div class="col-lg-6 col-md-6 col-sm-12">';
                    $html .= view('Frontend::services.apartment.items.grid-item', ['item' => $val])->render();
                    $html .= '</div>';
                }
            }
        }
    ...
    public function restoreApartment($request)
    {
        $params = $request->post('params', '');
        if (empty($params)) {
            return [
        ...
        restore_wishlist($post_id, GMZ_SERVICE_APARTMENT);
    ...
    public function hardDeleteApartment($request)
    {
        $params = $request->post('params', '');
        if (empty($params)) {
            return [
        ...
        $apartmentAvaiRepo->deleteByWhere([
            'post_id' => $post_id
        ]);

        hard_delete_wishlist($post_id, GMZ_SERVICE_APARTMENT);
    ...
    public function deletePost($request)
    {
        ...
        delete_wishlist($post_id, GMZ_SERVICE_APARTMENT);
    ...
    {
        $data = [
            'post_title' => 'New apartment ' . time(),
            'post_slug' => Str::slug('New apartment ' . time()),
            'author' => get_current_user_id(),
            'status' => 'temp'
        ];
    ...
    public function createSlug($data)
    {
        ...
        if (!empty($data['post_slug'])) {
            $isNewSlug = strpos($data['post_slug'], 'new-apartment-');
            if ($isNewSlug === false) {
    ...
    {
        $termRelationRepo = TermRelationRepository::inst();
        if (isset($post_data['apartment_type'])) {
            $all_types = get_terms('name', 'apartment-type', 'id');

            if (!empty($all_types)) {
                $type_in_str = '(' . implode(',', $all_types) . ')';
                $termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'apartment' AND term_id IN {$type_in_str}");
            }

            $apartment_type = $post_data['apartment_type'];
            if (!empty($apartment_type)) {
                $data_insert = [
                    'term_id' => $apartment_type,
                    'post_id' => $post_id,
                    'post_type' => 'apartment'
                ];
                $termRelationRepo->create($data_insert);
                $post_data['apartment_type'] = $apartment_type;
            } else {
                $post_data['apartment_type'] = '';
            }
        }

        if (isset($post_data['apartment_amenity'])) {
            $all_amenities = get_terms('name', 'apartment-amenity', 'id');
            if (!empty($all_amenities)) {
                $amenity_in_str = '(' . implode(',', $all_amenities) . ')';
                $termRelationRepo->deleteByWhereRaw("post_id = {$post_id} AND post_type = 'apartment' AND term_id IN {$amenity_in_str}");
            }

            $apartment_amenities = $post_data['apartment_amenity'];

            if (!empty($apartment_amenities) && is_array($apartment_amenities)) {
                foreach ($apartment_amenities as $item) {
                    $data_insert = [
                        'term_id' => $item,
                        'post_id' => $post_id,
                        'post_type' => 'apartment'
                ...
                $post_data['apartment_amenity'] = implode(',', $apartment_amenities);
            } else {
                $post_data['apartment_amenity'] = '';
            }
        }
    ...
    public function savePost($request)
    {
        ...
        if (!empty($post_id)) {
            ...
            $currentApartment = $this->repository->find($post_id);
            ...
            if (isset($post_data['status'])) {
                $current_post = get_post($post_id, GMZ_SERVICE_APARTMENT);
            ...
            //Update available table when edit Apartment
            if (isset($post_data['base_price'])) {
                $apartmentAvaiRepo->updateByWhere([
                    'post_id' => $post_id,
                    'price' => $currentApartment['base_price']
                ], [
            ...
            if ($updated) {
                ...
                if ($request->post('post_slug')) {
                    $response['permalink'] = get_apartment_permalink($updated['post_slug']);
                }

                $finish = $request->post('finish', '');
                if ($finish) {
                    $response['redirect'] = dashboard_url('edit-apartment/' . $post_id);
                }
